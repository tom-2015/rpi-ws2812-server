...
void set_pixel_color(thread_context* context, char* args) {
    ...
    if (is_valid_2D_channel_number(channel)) {
        ...
        if (led_channels[channel].surface != NULL) {
            ...
            pixels[y * led_channels[channel].pixel_stride + x] = convert_cairo_color((unsigned int) color);
            cairo_surface_mark_dirty_rectangle(led_channels[channel].surface, x, y, 1, 1);
        } else {
